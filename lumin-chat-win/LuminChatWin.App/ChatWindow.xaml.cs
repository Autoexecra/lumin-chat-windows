...
public partial class ChatWindow : Window, INotifyPropertyChanged
{
    ...
    {
        Conversation.Add(new ConversationItem("你", text, HorizontalAlignment.Right, new SolidColorBrush(Color.FromRgb(212, 226, 237))));
    }

    private void AddAssistantMessage(string text)
    {
        Conversation.Add(new ConversationItem("Lumin", text, HorizontalAlignment.Left, new SolidColorBrush(Color.FromRgb(242, 230, 215))));
        ScrollConversationToEnd();
    }

    private void AddSystemMessage(string text, bool success)
    {
        Conversation.Add(new ConversationItem(success ? "系统" : "系统提示", text, HorizontalAlignment.Left, new SolidColorBrush(success ? Color.FromRgb(224, 240, 229) : Color.FromRgb(248, 227, 227))));
        ScrollConversationToEnd();
    }

    private void AddToolLog(string title, string message, Brush brush)
    {
        ToolLogs.Insert(0, new ToolLogItem(title, message));
    }
    ...
    public sealed record ConversationItem(string Header, string Content, HorizontalAlignment Alignment, Brush Background);

    public sealed record ToolLogItem(string Title, string Message);
