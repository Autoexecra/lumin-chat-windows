...
`SSH 共享` 现在面向所有支持 bridge 的会话类型：

- PowerShell 默认分配 `25xx`
- SSH 默认分配 `24xx`
- Telnet 默认分配 `23xx`
- 串口继续使用 `22xx`

如果配置会话启用 `SSH 共享`，并且该类型支持 bridge，打开会话后会自动启动对应的 SSH bridge。

手工桥接入口仍保留在左侧 `共享` 标签中，但目标已扩展为所有可桥接会话。端口覆盖规则优先按稳定 descriptor 保存，避免依赖每次启动都会变化的运行时 `sessionId`；串口历史配置键仍兼容旧的 `COM12` 这类保存方式。
