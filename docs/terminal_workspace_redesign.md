...
- 正文渲染从纯文本改成 ANSI 富文本渲染，串口、SSH、Telnet、PowerShell 输出里的颜色和加粗信息会保留下来
...
1. `Agent` 标签：自动模式。用户输入需求后，Agent 会持续规划并自动下发终端命令，直到判断任务完成或明确要求人工补充信息。
2. `提示模式` 标签：只生成建议命令，不自动执行。用户点击执行后，会基于最新会话窗口内容立即继续下一轮规划。
3. `AI过程` 标签：实时显示规划、工具调用、工具结果和执行反馈。

新增行为：

- 模型下拉支持 `auto` 和具体 `levelN`
- 自动模式默认直接执行，不再保留建议命令框和自动执行复选框
- 提示模式保留建议命令框，用于人工审核和手动执行
- 每次发给 LLM 的终端上下文直接取当前会话窗口的最新字符串，并截断到最近 10000 个字符
- 终端 Agent 只开放与当前场景相关的工具：环境信息、网页搜索/抓取、知识库、SSH 远程控制；不开放本机文件和 git 等无关工具
...
- `LuminChatWin.App/TerminalAnsiRenderer.cs`
